        ...
        <div ref={sectionRef} className="w-full scroll-mt-24">
            ...
            <div className={`flex flex-wrap justify-center gap-3 mb-8 transition-all duration-1000 ${visibleSections.tabs ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                {tabs.map(tab => (
                    <button
                        ...
                        className={`flex items-center px-5 py-3 rounded-lg border transition-all duration-300 text-sm md:text-base 
                            ${activeTab === tab.id
                                ? 'bg-[#f97316] text-[#031a42] font-semibold scale-105 shadow-[0_0_15px_rgba(252,211,77,0.3)] border-[#f97316]'
                                : 'bg-white/70 border-[#fca74d]/50 text-slate-800 hover:bg-[#f97316]/80 hover:text-white'}`}
                    >
                        {tab.icon}
                        {tab.label}
                    </button>
            ...
            <div className="flex justify-center gap-2 mb-8">
                {tabs.map(tab => (
                    <div
                        key={`progress-${tab.id}`}
                        className={`h-1 rounded-full transition-all duration-500 ${activeTab === tab.id ? 'w-16 bg-[#f97316]' : 'w-6 bg-blue-900/80'}`}
                    ></div>
                ))}
            </div>
            ...
            <div className={`mx-auto max-w-arka bg-blue-900/20 p-6 md:p-12 rounded-xl border border-[#fca74d]/40 shadow-2xl transition-all duration-1000 min-h-[550px] md:min-h-[450px] flex flex-col justify-center ${visibleSections.content ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                ...
                {/* Vision & Mission */}
                {activeTab === 'mission' && (
                    <div className="animate-fadeIn relative w-full h-full flex flex-col justify-center min-h-inherit">

                        {/* 1. Background Decorative Icon */}
                        <div className="absolute right-4 md:right-10 top-1/2 -translate-y-1/2 opacity-[0.04] pointer-events-none hidden lg:block">
                            <Target size={320} strokeWidth={1} />
                        </div>

                        {/* 2. Text Content Container - Added relative z-10 to stay above the icon */}
                        <div className="space-y-10 relative z-10 max-w-3xl">
                            <div>
                                <h2 className="text-2xl font-bold text-[#f97316] mb-4 border-b border-[#fca74d]/20 pb-2 uppercase tracking-wide">
                                    Vision
                                </h2>
                ...
                {activeTab === 'objectives' && (
                    <div className="animate-fadeIn">
                        <h2 className="text-2xl font-bold text-[#f97316] mb-6">Our Objectives</h2>
                        <ul className="flex flex-col gap-4">
                            {[
                                ...
                                <li key={i} className="flex items-start group">
                                    <div className="mr-4 mt-1 bg-[#f97316] rounded-full p-1 text-[#031a42] group-hover:scale-110 transition-transform">
                ...
                {activeTab === 'usp' && (
                    <div className="animate-fadeIn">
                        <h2 className="text-2xl font-bold text-[#f97316] mb-8">ARKA Unique Selling Points</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6">
                            {[
                                ...
                                <div key={i} className="bg-blue-900/40 p-6 rounded-lg border-l-4 border-[#f97316] hover:bg-blue-800/60 transition-colors">
                                    <div className="text-[#f97316] mb-3">{usp.icon}</div>
                                    <h3 className="text-xl font-semibold text-[#fca74d] mb-2">{usp.title}</h3>
                                    <p className="text-slate-100 text-sm md:text-base leading-relaxed">{usp.desc}</p>
                                </div>
                            ))}
                        </div>
                        <div className="mt-10 text-center">
                            <p className="years-highlight-legacy inline-block bg-[#bd8409] text-[#031a42] px-6 py-3 rounded-lg text-lg font-bold">
                ...
                {activeTab === 'values' && (
                    <div className="animate-fadeIn">
                        <h2 className="text-2xl font-bold text-[#f97316] mb-8">Core Values</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {[
                                ...
                                <div key={i} className="bg-blue-900/60 p-6 rounded-lg border border-[#fca74d]/20 flex flex-col items-center text-center hover:translate-y-[-5px] hover:shadow-lg transition-all">
                                    <div className="w-16 h-16 rounded-full bg-[#f97316]/10 flex items-center justify-center mb-4 text-[#f97316] border-2 border-[#f97316]/30">
                                    ...
                                    <h3 className="text-lg font-semibold text-[#f97316] mb-2">{val.title}</h3>
